...
Toda señal par módulo es periódica con periodo $r$ y simétrica módulo $r$. La demostración de lo primero es idéntica a la de la \Cref{prop:mod->per}.
...
De acuerdo a la \Cref{prop:real}, como $x$ sólo toma valores reales entonces $X$ sólo toma valores reales. Se puede observar en la \Cref{fig:plot2} que $X[1] = X[2] = 2$ y $X[3] = 5$. Calculando la transformada de forma clásica usando la fórmula \eqref{eq:analysis}, se tiene
\begin{align*}
