...
\item Por definición se tiene que $\sigma(k) = \sum_{d \mid k} d$. Además $\mu(k) \le 1$ para todo $k \in \mathbb{N}$, luego
    ...
    \end{equation*}
\item Defínase, para $r \in \mathbb{N}$ fijo, la función aritmética
    \begin{equation*}
        \eta_r(n) = \begin{cases}
        ...
        \end{cases}
    \end{equation*}
Suponga ahora $n_0 \in \mathbb{N}$ arbitrario pero fijo. Se tiene que la función aritmética $g$ deifnida como $g(r) = \eta_r(n_0), \forall k \in \mathbb{N}$, es multiplicativa. En efecto, si $r,s \in \mathbb{N}$ son tales que $(r,s)=1$, entonces
\begin{equation*}
...
\begin{align*}
    (\mu * g)(r) & = \sum_{d \mid r} \mu \left( \frac{r}{d} \right) g(d) = \sum_{d \mid r} \mu \left( \frac{r}{d} \right) \eta_d(n_0) \\
                 & = \sum_{\substack{d \mid r \\ d \mid n_0}} \mu \left( \frac{r}{d} \right) d = \sum_{d \mid (n_0,r)} \mu \left( \frac{r}{d} \right) d = c_r(n_0) = f(r).
\end{align*}
...
De la demostración del \cref{it:ram1} se puede rescatar el siguiente corolario, usando la inversión de Möbius (\Cref{cor:mob1}).
